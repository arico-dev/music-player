...
├── data/
│   ├── local/                 Room: MusicDatabase, Entities (songs/albums/artists/genres/song_genres)
│   │   └── dao/               AlbumDao, ArtistDao, GenreDao, SongDao, SongGenreDao
│   ├── mediastore/            MediaStoreScanner (sonidos/albumes/generos) y MetadataReader (tags FLAC/MP3)
...
│   ├── home/                  HomeScreen
│   ├── library/               LibraryScreen + LibraryViewModel (tabs Canciones/Álbumes/Artistas/Géneros/Carpetas)
...
- Rutas: `home`, `library`, `search`, `settings`, `player`, y los detalles `album/{id}`, `artist/{id}`, `genre/{id}`, `folder?folderPath=...`.
- `fullScreenRoutes` = `{PLAYER, ALBUM, ARTIST, GENRE, FOLDER}`; al estar en ellas se oculta bottom bar y miniplayer.
...
- Selección: preferir la coincidencia por álbum (misma normalización), luego synced sobre plain.
